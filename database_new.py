...
from configuration import TIMEZONE, DATABASE_FILE
...
class Database:
    ...
    def get_all_channel_creations(self, guild_id: int) -> List[Tuple[Any, ...]]:
        ...
        try:
            self.c.execute(
                'SELECT * FROM channel_creation WHERE guild_id = ?', 
                (guild_id,)
            )
            results = self.c.fetchall()
            logger.debug(f"Retrieved {len(results)} channel creation configs for guild {guild_id}")
            return results
    ...
    def delete_channel_creation(self, id: int, guild_id: int) -> bool:
        ...
        try:
            ...
            if self.c.rowcount > 0:
                logger.info(f"Deleted channel creation config {id} for guild {guild_id}")
                return True
            else:
                logger.warning(f"No channel creation config found with id {id} for guild {guild_id}")
                return False
    ...
    def update_channel_lock(self, guild_id: int, channel_id: int, lock_in: int) -> bool:
        ...
        try:
            ...
            if self.c.rowcount > 0:
                logger.info(f"Updated channel lock for channel {channel_id} in guild {guild_id}")
                return True
            else:
                logger.warning(f"No channel lock found for channel {channel_id} in guild {guild_id}")
                return False
    ...
    def add_channel_lock(
        ...
        try:
            ...
            self.conn.commit()
            logger.info(f"Added/updated channel lock for channel {channel_id} in guild {guild_id}")
            return True
    ...
    def delete_channel_lock(self, guild_id: int, channel_id: int) -> bool:
        ...
        try:
            ...
            if self.c.rowcount > 0:
                logger.info(f"Deleted channel lock for channel {channel_id} in guild {guild_id}")
                return True
            else:
                logger.warning(f"No channel lock found for channel {channel_id} in guild {guild_id}")
                return False
