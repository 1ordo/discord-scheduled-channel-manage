    ...
    try:
        ...
        if action == 1:  # Set configuration
            await handle_set_action(
                interaction, guild_id, category, daily_time, delete_after,
                lock_after, channel_name, channel_text, channel_role
            )
...
async def handle_show_action(
    ...
    for i, config in enumerate(configs[:10]):  # Limit to 10 to avoid embed limits
        try:
            (config_id, guild_id, category_id, daily_creation_time, 
             ...
             channel_role, lock_after_minutes, created_at) = config
        except ValueError:
            # Handle older database format without created_at
            (config_id, guild_id, category_id, daily_creation_time, 
             close_after_minutes, channel_name, channel_text, 
             channel_role, lock_after_minutes) = config
    ...
    if len(configs) > 10:
        embed.set_footer(text=f"Showing 10 of {len(configs)} configurations. Use pagination for more.")
    else:
        embed.set_footer(text=f"Timezone: {TIMEZONE}")
